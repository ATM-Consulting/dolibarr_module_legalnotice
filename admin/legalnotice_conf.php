...
if (preg_match('/set_(.*)/',$action,$reg))
{
    $code=$reg[1];
    $val = GETPOST($code);
    if($code == 'LEGALNOTICE_MULTI_NOTICE_PROPAL') _setVisibilityExtrafields('legalnotice_selected_notice', 'propal',$val);
...
$var=!$var;
print '<tr '.$bc[$var].'>';
print '<td>'.$langs->trans("ManageMultiNoticePropal").'</td>';
print '<td align="center" width="20">&nbsp;</td>';
print '<td align="right" width="300">';
print '<form method="POST" action="'.$_SERVER['PHP_SELF'].'">';
print '<input type="hidden" name="token" value="'.$newToken.'">';
print '<input type="hidden" name="action" value="set_LEGALNOTICE_MULTI_NOTICE_PROPAL">';
print $form->selectyesno("LEGALNOTICE_MULTI_NOTICE_PROPAL",getDolGlobalString('LEGALNOTICE_MULTI_NOTICE_PROPAL'),1);
...
function _setVisibilityExtrafields($extrafields,$elementtype, $value) {
    ...
    $sql = 'UPDATE '.MAIN_DB_PREFIX.'extrafields SET list='.$value.' WHERE name="'.$extrafields.'" AND elementtype="'.$elementtype.'"';
    $db->query($sql);
